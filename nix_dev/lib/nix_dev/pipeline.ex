defmodule Nix.Dev.Pipeline do
  ...
  finishes, its result is collected and an attempt is made to start a new set of
  `opts[:start_fn]` for nodes that now have all their dependencies collected.
  ...
  defp loop({pending, running, collecting, finished}, opts) do
    {pending, new_running} = run_next(pending, running, finished, opts)
    {still_running, collecting} = collect_next(new_running, collecting)

    if collecting do
      receive do
        {:finished, payload, result} ->
          {pending, collecting, finished} = finish(payload, result, pending, collecting, finished)
          loop({pending, still_running, collecting, finished}, opts)
      end
    else
      {Enum.reverse(finished), pending}
    end
  end
